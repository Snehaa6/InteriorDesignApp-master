<%@ include file="cartnullqty.jsp" %>
	
	<div style='margin-left:50% '>
	
<b>	<a href="cartnull.jsp" class="w3-bar-item w3-button" ><img src = images/cart.png height=40px alt=Cart></a>

	<b style="position: relative;
  top: 16px;
  right: 37px;"><span style="height: 25px;
  width: 25px;
  background-color: red;
  border-radius: 50%;
  display: inline-block"><b style="color:white"><center><%=tqty %></center></b></span></b>
	</b>	
	
	<a href="customer_reg.jsp" class="w3-bar-item w3-button"><b>Customer Registration</b></a>
	
	<a href="customerlogin.jsp" class="w3-bar-item w3-button"><b>Customer Login</b></a>
	
	<a href="adminlogin.jsp" class="w3-bar-item w3-button"><b>Admin Login</b></a>
	
	<a href="contactus.jsp" class="w3-bar-item w3-button"><b>Contact Us</b></a>
	
	
	</div>
	
	<center>
	
	<b class="w3-wide" style="width:250px"> <h3><b>Online Interior Design</b></h3></b>
	
	<div class=" w3-large w3-text-grey" style="font-weight:bold">
	
	<a href="index.jsp" class="w3-bar-item w3-button"  >Home</a>&ensp;
	
	<a href="category.jsp" class="w3-bar-item w3-button">View Category</a>&ensp;
	
	<a href="tv.jsp" class="w3-bar-item w3-button">Bedroom</a>&ensp;
	<a href="laptop.jsp" class="w3-bar-item w3-button">Kitchen</a>&ensp;
	<a href="mobile.jsp" class="w3-bar-item w3-button">Hall</a>&ensp;
	<a href="watch.jsp" class="w3-bar-item w3-button">Bathroom</a>&ensp;
	
	<a href="viewproduct.jsp" class="w3-bar-item w3-button">View All Product</a>&ensp;
	
	<a href="aboutus.jsp" class="w3-bar-item w3-button">About Us</a>&ensp;
	
	</div>
	<hr>
	
